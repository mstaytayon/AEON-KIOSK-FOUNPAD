...
public class BookingSummaryActivity extends BaseActivity implements BaseActivity.Listener {
...
//    private void saveOfflineListOfChildren(int type,String transactionDate){
//        for(String elements : listOfChildren){
//            new CustomerNameDao(BookingSummaryActivity.this).save(referenceNumberID,referenceNumber,elements,type,transactionDate);
//        }
//    }
    ...
    private void printReceipt(BookingInformationModel model){
        ...
        byte[] esc_bytes = PrinterCmdUtils.Print(model,bitmap,totalChildrenWithPayment,totalAdditionalWithPayment,totalSocksAmount,totalAmount,BookingSummaryActivity.this,printAll);
    ...
    private void test(BookingInformationModel model){
        ...
        if (model.getCardNumber() ==  null){
            CardNumber = "Non Member";
        }
        try {
            SimpleDateFormat sdf = new SimpleDateFormat("MM/dd/yyy hh:mm:ss a");
            sb.append(StringUtils.center("BOOKING TRANSACTION", max));
            sb.append(StringUtils.leftPad("", max, "-"));

            sb.append("DATE:" + StringUtils.leftPad(sdf.format(new Date()), max - "DATE:".length(), " "));

            sb.append(StringUtils.leftPad("", max, "-"));
            sb.append("LOYALTY CARD#:" + StringUtils.leftPad(CardNumber, max - "LOYALTY CARD#:".length(), " "));
            sb.append("NAME:" + StringUtils.leftPad(model.getCustomerName(), max - "NAME:".length(), " "));
            sb.append(StringUtils.leftPad("", max, "-"));

            if(printAll == true){
                sb.append("NO. OF CHILDREN:" + StringUtils.leftPad(String.valueOf(model.getChildrenCount()), max - "NO. OF CHILDREN:".length(), " "));
                sb.append("NO. OF ADULT:" + StringUtils.leftPad(String.valueOf(model.getAdultCount()), max - "NO. OF ADULT:".length(), " "));
                sb.append(StringUtils.leftPad("", max, "-"));
            ...
            sb.append("CHILDREN AMOUNT:" + StringUtils.leftPad(new DecimalFormat("#,##0.00").format(totalChildrenWithPayment), max - "CHILDREN AMOUNT:".length(), " "));
            sb.append("ADDITIONAL AMOUNT:" + StringUtils.leftPad(new DecimalFormat("#,##0.00").format(totalAdditionalWithPayment), max - "ADDITIONAL AMOUNT:".length(), " "));
            sb.append("SOCKS AMOUNT:" + StringUtils.leftPad(new DecimalFormat("#,##0.00").format(totalSocksAmount), max - "SOCKS AMOUNT:".length(), " "));
            sb.append("TOTAL AMOUNT:" + StringUtils.leftPad(new DecimalFormat("#,##0.00").format(totalAmount), max - "TOTAL AMOUNT:".length(), " "));
            sb.append(StringUtils.leftPad("", max, "-"));
            sb.append("RRN:" + StringUtils.leftPad(model.getReferenceNumber(), max - "RRN:".length(), " "));
            sb.append(StringUtils.leftPad("", max, "-"));
        ...
        PrinterCmdUtils.testPrintsb(sb,bitmap);
    }
}
